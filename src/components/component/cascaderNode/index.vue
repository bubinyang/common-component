...
export default {
  ...
  created() {
    ...
    // }
    if (this.modeType(this.valType) === 'typeString' && Object.prototype.toString.call(this.value) === '[object Array]') {
      console.error('双绑数据类型应该为字符串，请检查')
    } else if (this.modeType(this.valType) === 'typeArray' && Object.prototype.toString.call(this.value) === '[object String]') {
      console.error('双绑数据类型应该为数组，请检查')
    }
  },
  methods: {
    handleChange(val) {
      this.$emit('change', this.modeType(this.valType) === 'typeString' ? val[val.length - 1] : val)
    },
