...
export default {
  ...
  watch: {
    dateOption: { // 深度监听，可监听到对象、数组的变化
      handler(val, oldVal) {
        this.timeValueFrom = val.date ? this.$moment(new Date(val.date)).format('HH:mm:ss') : '',
        this.timeValueTo = val.dateOther ? this.$moment(new Date(val.dateOther)).format('HH:mm:ss') : '';
      },
      deep: true // true 深度监听
    }
  },
  ...
  methods: {
    ...
    setInitialVal(val = new Date()) {
      if (this.dateOption.type === 'realtime') {
        this.dateOption.date = `${moment(val).subtract(this.setDiffVal('day')['startDiffVal'], 'days').format('YYYY-MM-DD')} ${this.timeValueFrom}`;
        this.dateOption.dateOther = `${moment(val).subtract(this.setDiffVal('day')['endDiffVal'], 'days').format('YYYY-MM-DD')} ${this.timeValueTo}`;
      } else if (this.dateOption.type === 'day') {
        this.dateOption.date = moment(val).subtract(this.setDiffVal('day')['startDiffVal'], 'days').format('YYYY-MM-DD');
        this.dateOption.dateOther = moment(val).subtract(this.setDiffVal('day')['endDiffVal'], 'days').format('YYYY-MM-DD');
      } else if (this.dateOption.type === 'month') {
        this.dateOption.date = moment(val).subtract(this.setDiffVal('month')['startDiffVal'], 'month').format('YYYY-MM');
        this.dateOption.dateOther = moment(val).subtract(this.setDiffVal('month')['endDiffVal'], 'month').format('YYYY-MM');
      } else {
        this.dateOption.date = moment(val).subtract(this.setDiffVal('years')['startDiffVal'], 'years').format('YYYY');
        this.dateOption.dateOther = moment(val).subtract(this.setDiffVal('years')['startDiffVal'], 'years').format('YYYY');
      }
    },
    ...
    setTimeFormatter() {
      return {
        realtime: (val, key) => {
          if (key === 'date') this.dateOption.date = `${moment(val).format('YYYY-MM-DD')} ${this.timeValueFrom}`;
          if (key === 'dateOther') this.dateOption.dateOther = `${moment(val).format('YYYY-MM-DD')} ${this.timeValueTo}`;
        },
        day: (val, key) => {
...
<style  lang="scss">
  .yearDateComponentPriva{
     display: flex;
     align-items: center;
     .septalLine{margin:0 5px}

     .el-radio-group{margin-right:5px;}
