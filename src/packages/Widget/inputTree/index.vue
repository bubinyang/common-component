...
export default {
  ...
  watch: {
    deptListVisible: {
      ...
      handler (val) {
        if (val) {
          this.filterText = ''
          const findVal = this.spreadData.find(item => item[this.valueKey]=== this.value || item.name === this.value)
          this.$nextTick(() => {
            if (findVal) { this.$refs.deptListTree.setCurrentKey(findVal[this.valueKey]) } else {
    ...
    origin: {
      handler (val) {
        ...
        this.findVal = this.spreadData.find(item => item[this.valueKey] === this.value || item.name === this.value)
  ...
  methods: {
    ...
    changeCascader () {
      this.$emit('change', { list: this.originVal, val: this.originVal[this.originVal.length - 1] })
    }
