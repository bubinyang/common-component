...
export function getTimeList({ time, type, diffTime, star = 0, end }) {
  ...
  // 起始 1号，结束是31号
  function getMonth({ starMonth, end }) { // 获取某月天数
    const timeData = [];
    ...
    for (let i = 0; i < Math.ceil(range); i++) {
      timeData.push(
        moment(starMonth).clone().add(i, 'day').valueOf()
      );
    }
    return timeData;
  }

  function getYear() { // 获取某年月数
    const timeData = [];
    for (let i = 0; i < 12; i++) {
...
class UploadClass {
  ...
  changeVal(callback) {
    this.el.onchange = (e) => {
      ...
      this.el.value = null;
      axios.post(this.url, this.formData, this.configs)
        .then(res => {
          this.beforeUpload();
          const file = res.data;

          if (!this.limit) { // 添加图片次数(无限制,可以一直加)
...
function treeSwitch(origin, type, parentId = 'parentId', id = 'id', children = 'children') {
  ...
  const typeTree = {
    merge() { return mergeTree(rootList); },
    spread() { return spreadTree(origin); }
  };
...
Array.prototype.reduceFind = function(callback) {
  return this.reduce((total, item, index, array) => {
    ...
    if ((index === array.length - 1) && Object.prototype.toString.call(total) === '[object Array]' && total.length === 0) {
      total = undefined;
    }
    return total;
  }, []);
...
export function getWeek(type) {
  const weeks = [
    { code: 1, label: '一' }, { code: 2, label: '二' },
    { code: 3, label: '三' }, { code: 4, label: '四' },
    { code: 5, label: '五' }, { code: 6, label: '六' },
    { code: 0, label: '日' }
  ];
...
class SetDialogPosition {
  constructor(containElName, options = {
    defaultPosition: 'rt'
  }) {
